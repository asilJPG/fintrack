 ...
 * Checks user data against achievement conditions and unlocks badges.
 */
...
const ACHIEVEMENTS: AchievementDef[] = [
  {
    ...
    emoji: "🐣",
    name: "Первый шаг",
    desc: "Добавил первый расход",
    check: (d) => d.totalExpenses > 0,
  },
  {
    key: "saved_100",
    emoji: "💰",
  ...
  },
  {
    ...
    name: "Накопитель",
    desc: "Сберёг $500",
    check: (d) => d.savings >= 500,
  },
  {
    key: "under_budget",
    emoji: "🦸",
    name: "Бюджетный герой",
    desc: "Расходы < доходов за месяц",
    check: (d) => d.monthlyExpenses < d.monthlyIncome && d.monthlyIncome > 0,
  },
  {
    key: "savings_rate_20",
    emoji: "📈",
    name: "20% правило",
    desc: "Норма сбережений ≥ 20%",
    check: (d) => d.savingsRate >= 20,
  },
  {
    key: "goal_created",
    emoji: "🎯",
    name: "Целеустремлённый",
    desc: "Создал первую цель",
    check: (d) => d.goalsCount > 0,
  },
  {
    key: "ten_expenses",
    emoji: "✍️",
    name: "Трекер",
    desc: "Добавил 10+ транзакций",
    check: (d) => d.totalExpenses >= 10,
  },
  {
    key: "fifty_expenses",
    emoji: "📊",
    name: "Аналитик",
    desc: "Добавил 50+ транзакций",
    check: (d) => d.totalExpenses >= 50,
  },
  {
    key: "used_ai",
    emoji: "🤖",
    name: "AI Советник",
    desc: "Использовал AI анализ",
    check: (d) => d.usedAI,
  },
  {
    key: "receipt_scanned",
    emoji: "📷",
    name: "Сканер",
    desc: "Отсканировал первый чек",
    check: (d) => d.scannedReceipt,
  },
  {
    key: "all_categories",
    emoji: "🌈",
    name: "Разнообразный",
    desc: "Использовал 5+ категорий",
    check: (d) => d.uniqueCategories >= 5,
  },
  {
    key: "no_splurge",
    emoji: "🧘",
    name: "Дисциплина",
    desc: "Сберёг > 30% в этом месяце",
    check: (d) => d.savingsRate >= 30,
  },
];
...
export default function AchievementsPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq("user_id", user.id)
      .then(({ data }) => {
        setUnlocked(new Set(data?.map((a: any) => a.achievement_key) ?? []));
        setLoading(false);
      });
  }, [user]);
  ...
  useEffect(() => {
    ...
    const uniqueCats = new Set(expenses.map((e) => e.category_name)).size;
    const goalsCount = 0; // Would need to fetch goals; simplified here
    ...
    const data = {
      ...
      goalsCount,
      usedAI: false,
      scannedReceipt: expenses.some((e) => e.receipt_url),
  ...
  return (
    <div className="max-w-3xl space-y-5">
      <div className="card p-5 flex items-center gap-4">
        <div className="w-14 h-14 bg-yellow-400/15 rounded-2xl flex items-center justify-center text-2xl">
          🏆
        </div>
        <div>
