    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#1c1c26] border border-white/[0.12] rounded-2xl w-full max-w-md shadow-2xl animate-slide-up max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] flex-shrink-0">
          <h2 className="font-bold text-base">{editExpense ? '✏️ Редактировать' : '➕ Добавить транзакцию'}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.08] transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          {!editExpense && (
            <div>
              <label className="label flex items-center gap-1.5">
                <Zap className="w-3 h-3 text-yellow-400" /> Быстрый ввод
              </label>
              <div className="flex gap-2">
                ...
                />
                <button type="button" onClick={handleQuickParse} className="btn-primary px-4 py-2.5 text-sm">↵</button>
              </div>
              <p className="text-[11px] text-white/25 mt-1.5">Формат: <span className="text-white/40 font-mono">сумма категория заметка</span></p>
              <div className="flex items-center gap-3 mt-4">
                <div className="flex-1 h-px bg-white/[0.06]" />
                <span className="text-[11px] text-white/20 font-medium">или вручную</span>
                <div className="flex-1 h-px bg-white/[0.06]" />
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="input" type="date" value={form.date} max={todayStr()}
                onChange={e => setForm(f => ({ ...f, date: e.target.value }))} />
            </div>
            <div>
              <label className="label">Время</label>
              <input className="input font-mono" type="time" value={form.time}
