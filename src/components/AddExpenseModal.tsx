...
function defaultForm(): FormState {
  return { amount: '', type: 'expense', category: null, note: '', date: todayStr(), time: nowTimeStr() }
}
...
export default function AddExpenseModal({ open, onClose, editExpense }: Props) {
  ...
  const handleQuickParse = () => {
    ...
    if (!parsed) { setError('Формат: сумма категория заметка (пример: 2500 еда обед)'); return }
  ...
  const incomeCategories = categories.filter(c => c.id === 'salary' || c.name.toLowerCase().includes('доход'))
  const displayCategories = form.type === 'income'
    ? incomeCategories.concat(expenseCategories.filter(c => !incomeCategories.includes(c)))
    : expenseCategories
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#1c1c26] border border-white/[0.12] rounded-2xl w-full max-w-md shadow-2xl animate-slide-up max-h-[92vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          {!editExpense && (
            <div>
              ...
              <div className="flex gap-2">
                ...
                />
                <button type="button" onClick={handleQuickParse} className="btn-primary px-4 py-2.5 text-sm">
                  ↵ Разобрать
                </button>
              </div>
              <p className="text-[11px] text-white/25 mt-1.5">
                Формат: <span className="text-white/40 font-mono">сумма категория заметка</span>
              </p>
              <div className="flex items-center gap-3 mt-4">
                <div className="flex-1 h-px bg-white/[0.06]" />
                <span className="text-[11px] text-white/20 font-medium">или заполните вручную</span>
                <div className="flex-1 h-px bg-white/[0.06]" />
              </div>
            </div>
          )}
          ...
          <button
            className={clsx(
              'w-full justify-center py-3 text-base font-semibold rounded-xl transition-all flex items-center gap-2',
              form.type === 'income'
...
export default function AddExpenseModal({ open, onClose, editExpense }: Props) {
  ...
  const incomeCategories = categories.filter(c => c.id === 'salary' || c.name.toLowerCase().includes('доход') || c.name.toLowerCase().includes('зарплат'))
  const expenseCategories = categories.filter(c => !incomeCategories.includes(c))
  const displayCategories = form.type === 'income' ? incomeCategories.concat(expenseCategories) : expenseCategories.concat(incomeCategories)
