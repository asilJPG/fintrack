...
const ACHIEVEMENTS: AchievementDef[] = [
  ...
  { key: 'saved_100',         emoji: '💰', name: 'Первая сотня',      desc: 'Сберёг $100',                     check: d => d.savings >= 100 },
  { key: 'saved_500',         emoji: '💎', name: 'Накопитель',        desc: 'Сберёг $500',                     check: d => d.savings >= 500 },
  ...
  { key: 'used_ai',           emoji: '🤖', name: 'AI Советник',       desc: 'Использовал AI анализ',           check: d => d.usedAI },
...
export default function AchievementsPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('achievements').select('achievement_key').eq('user_id', user.id)
      .then(({ data }) => {
        setUnlocked(new Set(data?.map(a => a.achievement_key) ?? []))
        setLoading(false)
      })
  }, [user])
  ...
  useEffect(() => {
    ...
    ACHIEVEMENTS.forEach(async a => {
      if (!unlocked.has(a.key) && a.check(data)) {
        await supabase.from('achievements').upsert({ user_id: user.id, achievement_key: a.key })
      ...
      <div className="card p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-white/40">
            Разблокировано <span className="text-yellow-400 font-bold">{unlockedCount}</span> из {ACHIEVEMENTS.length}
          </p>
        </div>
        {/* Progress bar */}
        <div className="flex-1 ml-4 hidden sm:block">
          ...
          <p className="text-xs text-white/30 mt-1">{Math.round((unlockedCount / ACHIEVEMENTS.length) * 100)}% завершено</p>
        </div>
      </div>
        ...
        {ACHIEVEMENTS.map(a => {
          ...
          return (
            <div key={a.key} className={clsx(
              ...
              <p className={clsx('font-bold text-sm mb-1', isUnlocked ? 'text-white' : 'text-white/40')}>{a.name}</p>
